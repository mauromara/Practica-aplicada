...
<?php
    ...
    $query = "SELECT id_usuario, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido FROM usuarios WHERE id_tipo_usuario = 3";
...
<body>
	<div id = "contenedor">
		...
		<main class="mainRegistro">
			...
			<form action="programar_cita.php" method="post" id="cdr">
                    <td><select name="busca" style="width: 200px; height: 25px;">
                        <option value="0">Seleccione Profesional</option> 
                        <?php WHILE($row = $resultado->fetch_assoc()) { ?>
                            <option value="<?php echo $row['id_usuario'] ?>"><?php echo $row['primer_apellido']. " " .$row['segundo_apellido']. " ". $row['primer_nombre']. " ". $row['segundo_apellido']?></option> 
                        <?php } ?>
                         </select>
                    </td>
